...
const s3=async function(req, res){
    ...
    try{
        ...
        if(files && files.length>0){
            ...
            let book=await booksModel.findOneAndUpdate({_id:bookId},{$set:{bookCover:uploadedFileURL}},{new:true})
            res.status(201).send({msg: "file uploaded succesfully", data: book})
        }
        else{
